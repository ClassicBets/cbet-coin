...
CzCBETWallet::CzCBETWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zCBET seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCBETSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCBETSeed_deprecated()) {
                    LogPrintf("%s: Updated zCBET seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zCBET seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zCBET wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCBETWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCBETCount(nCountLastUsed);
    else if (!walletdb.ReadZCBETCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCBETWallet::Lock()
{
    seedMaster = 0;
}

void CzCBETWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCBETWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCBET(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCBETWallet::LoadMintPoolFromDB()
{
...
void CzCBETWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCBETWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCBETWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcbetTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCBETWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCBET(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcbetTracker which also adds to database
    pwalletMain->zcbetTracker->Add(dMint, true);
...
void CzCBETWallet::SeedToZCBET(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCBETWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCBETWallet::GenerateDeterministicZCBET(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCBETWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCBETWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
