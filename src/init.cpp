...
#include "config/classicbets-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("classicbets-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "classicbets.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "classicbetsd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the CBET and zCBET money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 7777, 17777));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CBET/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CBET/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, classicbets, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CBET/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ClassicBets specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-cbetstake=<n>", strprintf(_("Enable or disable staking functionality for CBET inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcbetstake=<n>", strprintf(_("Enable or disable staking functionality for zCBET inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:7777"));
    ...
    strUsage += HelpMessageOpt("-backupzcbet=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCBET minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcbetbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zCBET backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeclassicbetsamount=<n>     " + strprintf(_("Keep N CBET anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 7776, 17776));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The ClassicBets Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("classicbets-loadblk");
...
/** Sanity checks
 *  Ensure that ClassicBets is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ClassicBets Core is shutting down."));
...
#endif
    // Make sure only a single ClassicBets process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ClassicBets Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ClassicBets version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //ClassicBets: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //ClassicBets specific: zerocoin and spork DB's
                ...
                // ClassicBets: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ClassicBets Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ClassicBets Core to complete") << "\n";
        ...
        zwalletMain = new CzCBETWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zCBETWallet
        uiInterface.InitMessage(_("Syncing zCBET wallet..."));

        bool fEnableZPivBackups = GetBoolArg("-backupzcbet", true);
        pwalletMain->setZPivAutoBackups(fEnableZPivBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zcbetTracker->Init();
    ...
    fEnableZeromint = true;
    nPreferredDenom = 5000;
    nZeromintPercentage = 1;
    if (chainActive.Height()>10055) { fEnableZeromint = false; }

// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeClassicBetsAmount = 2;
...
//    }
//
//    nAnonymizeClassicBetsAmount = GetArg("-anonymizeclassicbetsamount", 0);
//    if (nAnonymizeClassicBetsAmount > 999999) nAnonymizeClassicBetsAmount = 999999;
//    if (nAnonymizeClassicBetsAmount < 2) nAnonymizeClassicBetsAmount = 2;
    ...
    LogPrintf("Anonymize ClassicBets Amount %d\n", nAnonymizeClassicBetsAmount);
    ...
    /* Denominations
       ...
       For example:
       1CBET+1000 == (.1CBET+100)*10
       10CBET+10000 == (1CBET+1000)*10
    */
