...
// Copyright (c) 2018-2019 The ClssicBets Core developers
...
#if defined(NDEBUG)
#error "ClassicBets cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 30;
int64_t nReserveBalance = 0;

/** Fees smaller than this (in ucbet) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zCBET is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zCBET spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zCBET without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zCBET spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zCBET spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCBET spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcbet");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zCBET mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
     nSubsidy = 177057 * COIN;
    }
    if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 0) {
     nSubsidy = 1 * COIN;
    }
    else if (nHeight > Params().LAST_POW_BLOCK() && nHeight < 12161) {
     nSubsidy = 0.1 * COIN;
    }
    else if (nHeight > 12160 && nHeight < 22161) {
     nSubsidy = 1.0 * COIN;
    }    
    else if (nHeight > 22160 && nHeight < 32241) {
     nSubsidy = 1.5 * COIN;
    }   
    else if (nHeight > 32240 && nHeight < 42321) {
     nSubsidy = 1.75 * COIN;
    }  
    else if (nHeight > 42320 && nHeight < 52401) {
     nSubsidy = 2.0 * COIN;
    }
    else if (nHeight > 52400 && nHeight < 62481) {
     nSubsidy = 2.25 * COIN;
    } 
    else if (nHeight > 62480 && nHeight < 72561) {
     nSubsidy = 2.50 * COIN;
    } 
    else if (nHeight > 72560 && nHeight < 82641) {
     nSubsidy = 2.75 * COIN;
    } 
    else if (nHeight > 82640 && nHeight < 92721) {
     nSubsidy = 3.00 * COIN;
    } 
    else if (nHeight > 92720 && nHeight < 102801) {
     nSubsidy = 3.25 * COIN;
    } 
    else if (nHeight > 102800 && nHeight < 112881) {
     nSubsidy = 3.50 * COIN;
    } 
    else if (nHeight > 112880 && nHeight < 122961) {
     nSubsidy = 3.75 * COIN;
    } 
    else if (nHeight > 122960 && nHeight < 143121) {
     nSubsidy = 4.00 * COIN;
    } 
    else if (nHeight > 143120 && nHeight < 163281) {
     nSubsidy = 4.50 * COIN;
    }
    else if (nHeight > 163280 && nHeight < 183441) {
     nSubsidy = 5.00 * COIN;
    }
    else if (nHeight > 183440 && nHeight < 203601) {
     nSubsidy = 6.00 * COIN;
    }
    else if (nHeight > 203600 && nHeight < 213681) {
     nSubsidy = 5.75 * COIN;
    }
    else if (nHeight > 213680 && nHeight < 223761) {
     nSubsidy = 5.50 * COIN;
    }
    else if (nHeight > 223760 && nHeight < 233841) {
     nSubsidy = 5.25 * COIN;
    }
    else if (nHeight > 233840 && nHeight < 243921) {
     nSubsidy = 5.00 * COIN;
    }
    else if (nHeight > 243920 && nHeight < 254001) {
     nSubsidy = 4.75 * COIN;
    }
    else if (nHeight > 254000 && nHeight < 264081) {
     nSubsidy = 4.50 * COIN;
    }
    else if (nHeight > 264080 && nHeight < 274161) {
     nSubsidy = 4.25 * COIN;
    }
    else if (nHeight > 274160 && nHeight < 284241) {
     nSubsidy = 4.00 * COIN;
    }
    else if (nHeight > 284240 && nHeight < 294321) {
     nSubsidy = 3.75 * COIN;
    }
    else if (nHeight > 294320 && nHeight < 304401) {
     nSubsidy = 3.50 * COIN;
    }
    else if (nHeight > 304400 && nHeight < 314481) {
     nSubsidy = 3.25 * COIN;
    }
    else if (nHeight > 314480 && nHeight < 324561) {
     nSubsidy = 3.00 * COIN;
    }
    else if (nHeight > 324560 && nHeight < 334641) {
     nSubsidy = 2.75 * COIN;
    }
    else if (nHeight > 334640 && nHeight < 344721) {
     nSubsidy = 2.50 * COIN;
    }
    else if (nHeight > 344720 && nHeight < 354801) {
     nSubsidy = 2.25 * COIN;
    }
    else if (nHeight > 354800 && nHeight < 364881) {
     nSubsidy = 2.00 * COIN;
    }
    else if (nHeight > 364880 && nHeight < 374961) {
     nSubsidy = 1.75 * COIN;
    }
    else if (nHeight > 374960) {
     nSubsidy = 1.50 * COIN;
    }

    if (nHeight == 27000) { nSubsidy = 77 * COIN; }
    if (nHeight == 37000) { nSubsidy = 77 * COIN; }
    if (nHeight == 47000) { nSubsidy = 77 * COIN; }
    if (nHeight == 57000) { nSubsidy = 77 * COIN; }
    if (nHeight == 67000) { nSubsidy = 77 * COIN; }
    if (nHeight == 77000) { nSubsidy = 77 * COIN; }
    if (nHeight == 87000) { nSubsidy = 77 * COIN; }
    if (nHeight == 97000) { nSubsidy = 77 * COIN; }
    if (nHeight == 107000) { nSubsidy = 77 * COIN; }
    if (nHeight == 117000) { nSubsidy = 77 * COIN; }
    if (nHeight == 127000) { nSubsidy = 77 * COIN; }
    if (nHeight == 137000) { nSubsidy = 77 * COIN; }
    if (nHeight == 147000) { nSubsidy = 77 * COIN; }
    if (nHeight == 157000) { nSubsidy = 77 * COIN; }
    if (nHeight == 167000) { nSubsidy = 77 * COIN; }
    if (nHeight == 177000) { nSubsidy = 77 * COIN; }    
    if (nHeight == 187000) { nSubsidy = 77 * COIN; }
    if (nHeight == 197000) { nSubsidy = 77 * COIN; }
    if (nHeight == 207000) { nSubsidy = 77 * COIN; }
    if (nHeight == 217000) { nSubsidy = 77 * COIN; }
    if (nHeight == 227000) { nSubsidy = 77 * COIN; }
    if (nHeight == 237000) { nSubsidy = 77 * COIN; }
    if (nHeight == 247000) { nSubsidy = 77 * COIN; }
    if (nHeight == 257000) { nSubsidy = 77 * COIN; }
    if (nHeight == 267000) { nSubsidy = 77 * COIN; }
    if (nHeight == 277000) { nSubsidy = 77 * COIN; }
    if (nHeight == 287000) { nSubsidy = 77 * COIN; }
    if (nHeight == 297000) { nSubsidy = 77 * COIN; }
    if (nHeight == 307000) { nSubsidy = 77 * COIN; }
    if (nHeight == 317000) { nSubsidy = 77 * COIN; }
    if (nHeight == 327000) { nSubsidy = 77 * COIN; }
    if (nHeight == 337000) { nSubsidy = 77 * COIN; }
    if (nHeight == 347000) { nSubsidy = 77 * COIN; }
    if (nHeight == 357000) { nSubsidy = 77 * COIN; }
    if (nHeight == 367000) { nSubsidy = 77 * COIN; }
    if (nHeight == 377000) { nSubsidy = 77 * COIN; }

    if (nHeight == 25000) { nSubsidy = 177 * COIN; }
    if (nHeight == 50000) { nSubsidy = 277 * COIN; }
    if (nHeight == 75000) { nSubsidy = 377 * COIN; }
    if (nHeight == 100000) { nSubsidy = 477 * COIN; }
    if (nHeight == 125000) { nSubsidy = 577 * COIN; }
    if (nHeight == 150000) { nSubsidy = 677 * COIN; }
    if (nHeight == 175000) { nSubsidy = 777 * COIN; }
    if (nHeight == 200000) { nSubsidy = 777 * COIN; }
    if (nHeight == 225000) { nSubsidy = 777 * COIN; }
    if (nHeight == 250000) { nSubsidy = 777 * COIN; }
    if (nHeight == 275000) { nSubsidy = 777 * COIN; }
    if (nHeight == 300000) { nSubsidy = 777 * COIN; }
    if (nHeight == 325000) { nSubsidy = 777 * COIN; }
    if (nHeight == 350000) { nSubsidy = 777 * COIN; }
    if (nHeight == 375000) { nSubsidy = 777 * COIN; }
    if (nHeight == 400000) { nSubsidy = 777 * COIN; }
    if (nHeight == 425000) { nSubsidy = 777 * COIN; }
    if (nHeight == 450000) { nSubsidy = 777 * COIN; }
    if (nHeight == 475000) { nSubsidy = 777 * COIN; }
    if (nHeight == 500000) { nSubsidy = 777 * COIN; }
    if (nHeight == 525000) { nSubsidy = 777 * COIN; }
    if (nHeight == 550000) { nSubsidy = 777 * COIN; }
    if (nHeight == 575000) { nSubsidy = 777 * COIN; }
    if (nHeight == 600000) { nSubsidy = 777 * COIN; }
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    int64_t mNodeCoins = nMasternodeCount * 3333 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCBETStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ClassicBets
...
{
    RenameThread("cbet-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCBETMinted()
{
...
void RecalculateZCBETSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zCBET supply
...
bool RecalculateCBETSupply(int nHeightStart)
{
...
{
    // ClassicBets: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZCBETSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZCBETMinted();
        RecalculateZCBETSpent();
        RecalculateCBETSupply(Params().Zerocoin_StartHeight());
    }

    //Track zCBET money supply in the block index
    if (!UpdateZCBETSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zCBET supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CBET to zCBET
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCBET spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCBET serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zCBET stake block is less than allowed start height", __func__);

    if (CZPivStake* zCBET = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zCBET->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zCBET stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zCBET stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zCBET->GetValue()));
        if (nChecksum200 != zCBET->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zCBET->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZCBET() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zCBET fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCBET mints and %d zCBET spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ClassicBets: We use certain sporks during IBD, so check to see if they are
