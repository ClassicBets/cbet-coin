...
// Copyright (c) 2018-2019 The ClasicBets Core developers
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zcbetTracker->HasMintTx(hash))) {
            //zCBET stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZCBET;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by ClassicBets Address
...
bool IsZCBETType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZCBET:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZCBET || type == TransactionRecord::MNReward) {
