...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered classicbets address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CBET.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal classicbets:URI (classicbets:XyZ...?message=example)
            {
...
static void NotifyzCBETReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCBETReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCBETReset.connect(boost::bind(NotifyzCBETReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCBETReset.disconnect(boost::bind(NotifyzCBETReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcbetTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
