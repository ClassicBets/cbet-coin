...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::CBET, nValue);
...
{
    std::string genesisblockhash = "0000000f89360f33a11d291236edd7c385ec35e2306aa1b0fe45cdf4aa7ddd8d";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (classicbets.conf).");
            QMessageBox::warning(this, "ClassicBets Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
