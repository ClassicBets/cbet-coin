...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CBET);
    ...
    if (!settings.contains("nAnonymizeClassicBetsAmount"))
        settings.setValue("nAnonymizeClassicBetsAmount", 1000);

    nAnonymizeClassicBetsAmount = settings.value("nAnonymizeClassicBetsAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeClassicBetsAmount"))
        SoftSetArg("-anonymizecbetashamount", settings.value("nAnonymizeClassicBetsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in classicbets.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCBETashAmount:
            return QVariant(nAnonymizeClassicBetsAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCBETashAmount:
            nAnonymizeClassicBetsAmount = value.toInt();
            settings.setValue("nAnonymizeClassicBetsAmount", nAnonymizeClassicBetsAmount);
            emit anonymizeCBETashAmountChanged(nAnonymizeClassicBetsAmount);
            break;
        case CoinControlFeatures:
