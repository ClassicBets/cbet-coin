...
CzCBETTracker::CzCBETTracker(std::string strWalletFile)
{
...
CzCBETTracker::~CzCBETTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCBETTracker::Init()
{
...
bool CzCBETTracker::Archive(CMintMeta& meta)
{
...
bool CzCBETTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCBETTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCBETTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCBETTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCBETTracker::GetSerialHashes()
{
...
CAmount CzCBETTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCBETTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCBETTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCBETTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCBETTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCBETTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCBETTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCBETTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCBETTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCBETTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCBETTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCBETTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCBETTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCBETTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCBETTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCBETTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCBETTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzcbet from DB\n", __func__, listDeterministicDB.size());
    }
