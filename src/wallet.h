...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CBET at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCBET send
enum ZerocoinSpendStatus {
    ZCBET_SPEND_OKAY = 0,                            // No error
    ZCBET_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCBET_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCBET_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCBET_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCBET_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCBET_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCBET_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCBET_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCBET_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCBET_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCBET_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCBET_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCBET_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCBET_SPENT_USED_ZCBET = 14,                      // Coin has already been spend
    ZCBET_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCBET_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZCBETOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzcbetAuto) const;
    ...
    */
    ...
    CzCBETWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCBETTracker> zcbetTracker;
    ...
    void setZWallet(CzCBETWallet* zwallet)
    {
        zwalletMain = zwallet;
        zcbetTracker = std::unique_ptr<CzCBETTracker>(new CzCBETTracker(strWalletFile));
    }

    CzCBETWallet* getZWallet() { return zwalletMain; }
    ...
    /// Get 3333 CBET output and keys which can be used for the Masternode
    ...
    /** zCBET reset */
    boost::signals2::signal<void()> NotifyzCBETReset;
