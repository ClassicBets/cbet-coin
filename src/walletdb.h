...
public:
    ...
    bool WriteZCBETSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCBETSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCBETSeed_deprecated(uint256& seed);
    bool EraseZCBETSeed();
    bool EraseZCBETSeed_deprecated();

    bool WriteZCBETCount(const uint32_t& nCount);
    bool ReadZCBETCount(uint32_t& nCount);
