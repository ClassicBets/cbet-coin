...
// The zCBET block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCBET is a hash of the serial
...
{
    //Create an output returning the zCBET that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCBETOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCBET output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCBET", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCBETOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCBET output", __func__);
...
{
    CzCBETTracker* zcbetTracker = pwallet->zcbetTracker.get();
    CMintMeta meta;
    if (!zcbetTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcbetTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a CBET stake is the outpoint
